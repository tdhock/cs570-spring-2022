...
- For classification problems ($y\in\{0,1\}$) we use the logistic
  loss, resulting in a model called logistic regression.
- Confusing name because it is actually for classification problems,
  not regression problems! 
- Labels are converted to $\tilde y\in \{-1,1\}$, then logistic loss
  is $\ell(\beta + \mathbf w^T \mathbf x, y) = \log[1+\exp(-\tilde
  y(\beta + \mathbf w^T \mathbf x))]$.
...
- Start with random initial coefficients near zero.
