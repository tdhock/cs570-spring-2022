...
- $\nabla_\beta \mathcal L:\mathbb R^{p+1}\rightarrow\mathbb R$ tells
  ...
  \mathcal L(\mathbf w, \beta) = \partial L(\mathbf w, \beta)/\partial \beta$.
- $\nabla_{\mathbf w} \mathcal L:\mathbb R^{p+1}\rightarrow\mathbb
  R^p$ tells us how $\mathbf w$ is changing at the input coefficients,
  $\nabla_\beta \mathcal L(\mathbf w, \beta) = [\partial L(\mathbf w,
  \beta)/\partial w_1 \cdots \partial L(\mathbf w,
  \beta)/\partial w_p]$.
...
- Validation set used to learn hyper-parameter, number of iterations
  $k$. The best one minimizes error (maximizes accuracy) with respect
  to validation set.
- Plot subtrain and validation loss as a function of iteration $k$,
  you should see subtrain always decreasing, and validation U shape.
...
After learning it is most convenient for the user if you report the
