...
\begin{frame}
  ...
  \item Each edge in the computation graph is an operation.
  \item Only three operations needed: matrix multiply (\texttt{mm}),
    non-linear activation (\texttt{act}), and computing loss given
    predicted scores (\texttt{loss}).
  \end{itemize}
  ...
  \begin{tikzpicture}[->,>=latex,shorten >=1pt,auto,node distance=2.1cm,
    ...
    \node[main node] (hL) [right of=aL] {$h^{(L)}$};
    \node[main node] (y) [below of=hL] {$y$};
    \node[main node] (J) [right of=hL] {$J$};
    ...
    (y) edge [bend left] (J)
    (aL) edge node {\texttt{act}} (hL)
    (hL) edge node {\texttt{loss}} (J)
    ;
  \end{tikzpicture}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Each operation should define __init__ and backward methods.
  \item method __init__(node1, node2, ...) instantiates a new node in
