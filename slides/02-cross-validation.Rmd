...
def contour_paths(three_cols, contour_values):
    from skimage import measure
    two_cols = three_cols.iloc[:,:2]
    uniq_df = two_cols.apply(pd.unique)
    n_grid = uniq_df.shape[0]
    fun_mat = two_cols.to_numpy().reshape(
        [n_grid,n_grid]).transpose()
    contours = measure.find_contours(fun_mat, contour_values)
    contour_df_list = []
