...
That should create a new "cs570s22" python environment.
Before you activate an environment, you need to setup your shell for the first time.

#+begin_src shell-script
conda init bash
#+end_src

That should put some code in your ~/.bash_profile, I had to copy it to my ~/.bashrc to get it to work on my setup (git bash in emacs shell on windows). After restarting your shell you should see a (base) prefix in your prompt, which indicates the name of the currently activated conda environment.
...
To get python support in emacs I had to install emacs packages (pyvenv,elpy,conda) via M-x package-list-packages.
...
In emacs When editing a python file we can "activate" or "work on" the
conda environment via either
- M-x conda-env-activate RET cs570s22 RET
- M-x RET pyvenv-workon RET cs570s22 RET
then C-c C-z to get an interactive
